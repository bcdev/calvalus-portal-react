...
export interface ExternalObjectComponentProps<E, ES> {
    id: string;
 ...
 *   3. append the *container* element of the external object to the "div" element controlled by this
 *      PermanentComponent;
 ...
 * @version 0.4
 */
export abstract class ExternalObjectComponent<E, ES, P extends ExternalObjectComponentProps<E, ES>, S>
    ...
    protected shouldExternalObjectUpdate(prevState: ES | undefined, nextState: ES): boolean {
     ...
     * @param container The HTML element created by *newContainer()*, if any.
     */
    abstract updateExternalObject(object: E, prevState: ES | undefined, nextState: ES,
                                  parentContainer: HTMLElement | null, container: HTMLElement): void;
    ...
    // - componentWillReceiveProps() to do what componentWillUpdate() currently does AND set whether render()
    ...
    private updateExternalComponentAndSaveProps(nextProps: Readonly<P & ES>): void {
        ...
        if (this.props.debug) {
            console.log(
                'ExternalObjectComponent: updating existing external object with id =',
                this.props.id, prevState, nextState);
        }
    ...
    private get externalObjectStore(): ExternalObjectStore<E, ES> {
        return (this.props.externalObjectStore || ExternalObjectComponent.DEFAULT_EXTERNAL_OBJECT_CACHE) as
            ExternalObjectStore<E, ES>;
    }
}
